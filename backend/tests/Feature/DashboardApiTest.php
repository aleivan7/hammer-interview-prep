...
class DashboardApiTest extends TestCase
{
    ...
    #[TestDox('Dashboard period scopes cash flows and bucket actuals to the requested month')]
    public function test_dashboard_period_excludes_other_month_cash_flows_and_bucket_spend(): void
    {
        $user = $this->withDemoUser(User::factory()->average()->create());
        ...
        $response->assertOk()
            ...
            ->assertJsonPath('data.safe_to_spend.bucket_actuals.need', 5_000);

        $cashFlowNames = collect($response->json('data.cash_flows'))->pluck('name')->all();
        $this->assertSame(['July Paycheck'], $cashFlowNames);
        $this->assertSame(
            [],
            collect($response->json('data.safe_to_spend.unusual_alerts'))->pluck('merchant')->all(),
        );
    }
}
