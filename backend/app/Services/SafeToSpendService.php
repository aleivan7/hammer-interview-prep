...
final class SafeToSpendService
{
     ...
     * }
     */
    public function forUser(User $user, ?CarbonInterface $asOf = null): array
    {
        $asOf = Carbon::parse($asOf?->toDateString() ?? 'now')->startOfDay();
        $periodStart = $asOf->copy()->startOfMonth();
        $periodEnd = $asOf->copy()->endOfMonth();

        $plan = FinancialPlan::query()->forUser($user)->first() ?? new FinancialPlan([
            'needs_percent' => 50,
            'wants_percent' => 30,
            'savings_percent' => 20,
        ...
        $availableCashCents = (int) Account::query()->forUser($user)->sum('balance_cents');
        ...
        $spentByBucket = $this->spentByBucket($user, $periodStart, $periodEnd);
        ...
        return [
            ...
            'unusual_alerts' => $this->unusualAlerts($user, $periodStart, $periodEnd),
        ];
    }
     ...
     */
    private function spentByBucket(User $user, CarbonInterface $start, CarbonInterface $end): array
    {
        $totals = [
            'need' => 0,
            'want' => 0,
     ...
     */
    private function unusualAlerts(User $user, CarbonInterface $start, CarbonInterface $end): array
    {
